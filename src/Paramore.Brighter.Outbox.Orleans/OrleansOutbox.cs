        ...
        IAmAnOutboxViewerAsync<Message>
    {
        ...
        public async Task<Message> GetAsync(Guid messageId, int outBoxTimeout = -1, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                string result = await _grainFactory
                    .GetGrain<IMessageOutboxGrain>(messageId)
                    .Get();
            ...
            {
                s_logger.Value.ErrorFormat(
                        "OrleansOutbox/Get: An error occurred while retreiving the Command {0} from the Message Store : " + e.Message, messageId);
                throw;
            }
        }
        public Task<IList<Message>> GetAsync(int pageSize = 100, int pageNumber = 1, CancellationToken cancellationToken = default)
        {
