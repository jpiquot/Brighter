...
namespace Paramore.Brighter.Inbox.Orleans
{
    ...
    {
        private static readonly Lazy<ILog> SLogger = new Lazy<ILog>(LogProvider.For<OrleansInbox>);
        ...
        ///     An identifier for the context in which the command has been processed (for example, the name
        ///     of the handler)
        /// </param>
        ...
        {
            AddAsync(command, contextKey, timeoutInMilliseconds)
               .GetAwaiter()
               .GetResult();
        }
        ...
        {
            return GetAsync<T>(id, contextKey, timeoutInMilliseconds)
                  .GetAwaiter()
                  .GetResult();
        }
        ...
        {
            return ExistsAsync<T>(id, contextKey, timeoutInMilliseconds)
                  .GetAwaiter()
                  .GetResult();
        }
        ...
        public async Task AddAsync<T>
            (T command, string contextKey, int timeoutInMilliseconds = -1, CancellationToken cancellationToken = default)
            where T : class, IRequest
        {
            try
            {
                ...
                {
                    SLogger.Value.WarnFormat("OrleansInbox/Add: Timeout parameter is not supported. Value is ignored.");
                }
        ...
        public async Task<T> GetAsync<T>
            (Guid id, string contextKey, int timeoutInMilliseconds = -1, CancellationToken cancellationToken = default)
            where T : class, IRequest
        {
            try
            {
                if (timeoutInMilliseconds != -1)
                {
                    SLogger.Value.WarnFormat("OrleansInbox/Get: Timeout parameter is not supported. Value is ignored.");
                }

                CommandAndContext result = await _grainFactory.GetGrain<ICommandInboxGrain>(id)
                                                              .Get();
            ...
            catch (Exception e)
            {
                SLogger.Value.ErrorFormat
                    (
                     "OrleansInbox/Get: An error occurred while retreiving the Command {0} from the Message Store : "
                   + e.Message, id
                    );
            ...
            where T : class, IRequest
        {
            try
            {
                if (timeoutInMilliseconds != -1)
                {
                    SLogger.Value.WarnFormat("OrleansInbox/Exists: Timeout parameter is not supported. Value is ignored.");
                }

                return await _grainFactory.GetGrain<ICommandInboxGrain>(id)
            ...
            catch (Exception e)
            {
                SLogger.Value.ErrorFormat
                    (
                     "OrleansInbox/Exists: An error occurred while checking if Command {0} with ContextKey {1} exists in the Message Store : "
                   + e.Message, id, contextKey
                    );
